...
- **Setup screen:** paste text directly, or upload a `.txt` or `.pdf` file (PDF text is extracted client-side with [pdf.js](https://mozilla.github.io/pdf.js/), loaded from a CDN only when a PDF is actually selected). A slider sets reading speed from 200–800 WPM, with a live word count and estimated reading time.
- **Reading mode:** click "Start Reading" to go full-screen — plain black background, one word at a time in white, with the word's optimal recognition point (ORP) letter highlighted in red and horizontally fixed so your eyes don't move between words. The only visible control is an &times; in the corner to exit. In silent mode, longer words and sentence-ending punctuation get slightly more display time.
- **Read aloud (optional):** an on/off toggle on the setup screen uses the browser's built-in Web Speech API to speak each word as it's shown. With voice on, the *speech itself drives the pace* — a word only advances once it's finished being spoken — so the word on screen is always the word you hear, at a speaking rate scaled from the WPM slider. It degrades gracefully: if the browser doesn't support speech synthesis, the toggle is disabled with a note, and silent mode is unaffected.
- **Keyboard/mouse shortcuts in reading mode** (no on-screen buttons, to keep the screen distraction-free): click the word or press Space to pause/resume, Left/Right arrows to step a word at a time (also speaking it, if voice is on), Escape (or the &times;) to exit.
